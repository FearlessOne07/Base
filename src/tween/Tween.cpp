...
namespace Base
{
  ...
  {
    return static_cast<unsigned char>(glm::mix(start, end, t));
  };

  template <> Color Tween<Color>::TweenLerp(Color start, Color end, float t)
  {
    Color color;
    color.r = static_cast<unsigned char>(glm::mix(start.r, end.r, t));
    color.g = static_cast<unsigned char>(glm::mix(start.g, end.g, t));
    color.b = static_cast<unsigned char>(glm::mix(start.b, end.b, t));
    color.a = static_cast<unsigned char>(glm::mix(start.a, end.a, t));
    return color;
  };
} // namespace Base
